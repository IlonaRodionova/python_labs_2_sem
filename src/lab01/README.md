# Лабораторная работа 1 - Класс Product (Интернет-магазин)


### №1. Что является сущностью?

Товар (Product) - сущность в предметной области (Интернет магазин)

### №2. Какие атрибуты будут у данного объекта?

1. **Атрибут класса:** `currency` (валюта) - одинакова для всех товаров магазина.
2. **Атрибуты экземпляра:** Данные самого товара — название (`name`), базовая цена (`price`), остаток на складе (`stock`), текущая скидка (`discount`), категория (`category`) и уникальный артикул (`product_id`).
Для защиты данных реальные значения хранятся в *защищенных* полях (с нижним подчеркиванием, например `_price`), а доступ к ним снаружи предоставляется через публичные свойства (properties).

### №3. Какие инварианты?

Инварианты моего класса — это строгие бизнес-правила, которые объект не дает нарушить на протяжении всего своего жизненного цикла.
* Цена (`price`) всегда должна быть числом и строго больше нуля.
* Остаток на складе (`stock`) не может быть отрицательным.
* Скидка (`discount`) всегда лежит в диапазоне от 0 до 99%.
* Текстовые поля (имя, категория, ID) не могут быть пустыми строками.
Эти инварианты жестко контролируются модулем валидации через сеттеры. Объект просто не позволит записать в себя некорректные данные, выбросив ошибку.

### №4. Что значит равенство?

Равенство реализовано через магический метод `__eq__`. В моей архитектуре два объекта товара считаются равными **только при совпадении их уникального идентификатора (`product_id`)**.
Это значит, что даже если у товаров в моменте разная цена или остаток на складе (например, данные обновились), для системы это всё равно один и тот же товар, так как совпадает артикул. Если же попытаться сравнить товар с объектом другого класса (например, со строкой), метод безопасно вернет `False`.

### №5. Есть ли состояния?

Да, у объекта реализовано логическое состояние через защищенный флаг `_active` (активен / снят с продажи).
От этого состояния напрямую зависит поведение объекта: если товар переведен в неактивное состояние, бизнес-методы (такие как покупка `reduce_stock` или запрос цены `get_final_price`) блокируются и выбрасывают ошибку. При этом система может управлять состоянием автоматически: например, метод `update_available()` сам деактивирует товар, если его нет на складе.


## Реализованный класс

### Атрибуты экземпляра (закрытые поля)
- `_name` - название товара
- `_price` - цена товара
- `_stock` - количество на складе
- `_discount` - скидка в процентах
- `_category` - категория товара
- `_product_id` - идентификатор товара (read-only)
- `_active` - логическое состояние товара (активен/неактивен)

### Атрибут класса
- `currency` - валюта магазина (по умолчанию "₽")

### Свойства (properties)
Реализованы свойства с валидацией через `@property` и `@setter`:
- `name` - с валидацией типа и проверкой на пустую строку
- `price` - с валидацией типа (int/float) и диапазона (1-99999)
- `stock` - с валидацией типа (int) и проверкой (>= 0)
- `discount` - с валидацией типа (int) и диапазона (0-99), включает `@deleter`
- `category` - с валидацией типа и проверкой на пустую строку
- `product_id` - только чтение 

### Валидация данных (validate.py)
Реализованы функции:
- `validate_name(value)` - проверка типа str и непустой строки
- `validate_price(value)` - проверка типа (int/float) и диапазона
- `validate_stock(value)` - проверка типа int и неотрицательности
- `validate_discount(value)` - проверка типа int и диапазона 0-99
- `validate_category(value)` - проверка типа str и непустой строки
- `validate_product_id(value)` - проверка типа str и непустой строки

### Магические методы
- `__str__()` - строковое представление товара
- `__repr__()` - техническое представление для отладки
- `__eq__(other)` - сравнение товаров по `product_id`
- `__lt__(other)` - сравнение товаров по цене

### Бизнес-методы
- `get_final_price()` - возвращает цену с учетом скидки 
- `reduce_stock(quantity)` - уменьшает количество товара на складе (проверяет активность и достаточность)
- `update_available()` - обновляет состояние товара в зависимости от количества на складе
- `activate()` - выставляет товар на продажу
- `deactivate()` - снимает товар с продажи

### Логическое состояние объекта
Объект имеет состояние `_active`, которое влияет на поведение:
- `get_final_price()` работает только для активных товаров
- `reduce_stock()` блокирует операции для неактивных товаров
- `update_available()` автоматически меняет состояние при изменении количества

---

## Итог в demo.py

### 1. Создание объектов и строковое представление (__str__)
**Реализовано:** Создание двух объектов товаров и вывод через `print()`

![alt text](../../images/lab01/01.png)

### Некорректное создание объекта (валидация)
**Реализовано:** Попытка создать объект с некорректными данными (пустое имя, отрицательная цена, отрицательный stock, скидка > 99, пустые category и product_id)



![alt text](../../images/lab01/1.1.png)

### 2. Демонстрация валидации (через свойства и сеттеры)
**Реализовано:** Демонстрация работы валидации при попытке установить некорректные значения через сеттеры



![alt text](../../images/lab01/02.png)

### 3. Работа со свойствами и сеттерами
**Реализовано:** Успешное изменение свойств через сеттеры с валидацией



![alt text](../../images/lab01/03.png)

### 4. Магические методы (__eq__, __lt__, __repr__)
**Реализовано:** 
- Сравнение объектов через `__eq__` (по product_id)
- Сравнение через `__lt__` (по цене)
- Техническое представление через `__repr__`



![alt text](../../images/lab01/04.png)

### 5. Бизнес-логика и изменение состояния
**Реализовано:** 
- Получение финальной цены с учетом скидки
- Уменьшение количества товара на складе
- Автоматическое изменение состояния при обнулении количества
- Блокировка операций для неактивных товаров



![alt text](../../images/lab01/05.png)

### 6. Атрибут класса (Валюта)
**Реализовано:** Демонстрация изменения атрибута класса `currency`, который влияет на все экземпляры



![alt text](../../images/lab01/06.png)